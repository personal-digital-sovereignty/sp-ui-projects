<script lang="ts">
    ...
    let activeProjects = $derived(
        projectState.projects.filter((p) => !p.is_archived).length,
    );

    let totalTasks = $derived(
        projectState.projects.reduce(
            (acc, p) => acc + (p.tasks?.length || 0),
            0,
        ),
    );
    let avgTasksPerProject = $derived(
        totalProjects ? (totalTasks / totalProjects).toFixed(1) : "0",
    );

    // Real cycle time from backend telemetry, with fallback
    ...
    async function fetchCycleTime() {
        try {
            ...
            if (res.ok) {
                ...
                // avg_latency_ms is in milliseconds; convert to days for display
                const ms = data.avg_latency_ms ?? 0;
                if (ms > 0) {
                    const days = ms / (1000 * 60 * 60 * 24);
                    if (days >= 1) {
                        avgCycleTime = `${days.toFixed(1)} dias`;
                    } else {
                        const hours = ms / (1000 * 60 * 60);
                        avgCycleTime = `${hours.toFixed(1)} horas`;
                    }
                }
            }
        } catch {
            // Keep simulated fallback on API failure
        } finally {
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    ...
    >
        ...
        <div>
            ...
            <p
                class="text-2xl font-black text-slate-800 dark:text-slate-200 transition-colors"
            >
                {activeProjects}
                <span
